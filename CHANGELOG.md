...
### Added
- Mobile admin access: `MobileBottomNav` now renders a conditional 5th "Admin" tab (Shield icon, amber accent) when the current user's role is `admin` or `moderator`, so moderators on phones and tablet-portrait viewports can reach `/admin` without typing the URL. The desktop `DashboardNav` already gated the admin link this way at `lg+`; the bottom bar now mirrors that gate at `<lg`. Role is fetched via `GET /api/auth/me` on mount using the same shape as `DashboardNav` — regular users still see exactly the four original tabs, and the new tab disappears once inside `/admin` because the `(admin)` layout does not render the bottom nav (`src/components/dashboard/mobile-nav.tsx`)

### Changed
- Unify matching across the dashboard and explore/search pages onto a single Market Basket Analysis scoring path. Previously `/api/matches` used `calculateMatchScore` + `determineMatchType` while `/api/attendees/search` ran a separate inline point-based scorer and the explore card derived the HIGH AFFINITY vs STRATEGIC tag from a 50% percentage threshold — so the same user pair could render (for example) 20% HIGH AFFINITY on the matches page and 48% STRATEGIC on the search page. The explore route now calls the same `calculateMatchScore` + `determineMatchType` and surfaces the authoritative `matchType` on `AttendeeSearchResult`; the explore card consumes that field directly and `EXPLORE_HIGH_AFFINITY_MIN_PERCENT` is deleted (`src/app/api/attendees/search/route.ts`, `src/components/explore/attendee-card.tsx`, `src/components/explore/explore-container.tsx`, `src/types/index.ts`).
- Make match scores useful on sparse cohorts. Raw MBA scores compressed into the 15–35% band with only 9 testers, so "best match in the event" rendered as 30%. Added `rescaleCohortScores()` — a percentile-based rescale that preserves ranking order but stretches the visible band to `[0.10, 0.95]`, applied in both `/api/matches` and `/api/attendees/search` after filtering. Top matches now read ~95% and the floor reads ~10% regardless of how compressed the raw distribution is (`src/lib/matching/market-basket-analysis.ts`, `src/lib/matching/matching-service.ts`, `src/app/api/matches/route.ts`, `src/app/api/attendees/search/route.ts`).
- Add embedding cosine similarity as a semantic affinity signal in `calculateMatchScore`. The pgvector `profile_embedding` generated on every questionnaire submit was live but orphaned — neither `/api/matches` nor `/api/attendees/search` was reading it. Now both routes select `profile_embedding` alongside `questionnaire_data` and pass an optional `EmbeddingPair` into `calculateMatchScore`; cosine similarity is clamped from the typical sentence-embedding band `[0.3, 0.9]` into `[0, 1]` and added to affinity with a 0.2 weight. When either vector is missing the term contributes zero and the function falls through to the existing MBA-only path — no backfill script needed. Captures paraphrase and synonym overlap that the token-Jaccard text path misses ("machine learning" ≈ "ML engineering") (`src/lib/matching/market-basket-analysis.ts`, `src/lib/ai/cosine.ts`, `src/lib/ai/embeddings.ts`, `src/app/api/matches/route.ts`, `src/app/api/attendees/search/route.ts`).
- Extend `COMPLEMENTARY_PAIRS` beyond archetype so the STRATEGIC tag reflects real complementarity rather than falling-below-a-threshold. Added genuine-opposite pairs for team qualities (problem-solving ↔ ideas, collaboration ↔ perspective, energy ↔ perspective) and for summit rhythms (planner ↔ go-with-the-flow, early-bird ↔ night-owl, social ↔ recharge-solo). Strategic credit is now per-attribute: archetype complements remain the dominant signal at 0.8× weight, team-quality and personality-tag complements contribute a softer 0.45× so one opposite team quality doesn't push similar-profile pairs into the STRATEGIC tag. Bumped `TEXT_OVERLAP_WEIGHT` from 0.42 → 0.55 so free-text overlap weighs more when it fires, and removed the weight=0 `personalInterestPhoto` entry from `ATTRIBUTE_WEIGHTS` (dead config) (`src/lib/matching/market-basket-analysis.ts`, `src/lib/matching/market-basket-analysis.test.ts`).
...
### Fixed
- Gallery photo uploads no longer strand pending photos outside the moderation queue. `POST /api/profile/photos` was calling `addToModerationQueue()` as fire-and-forget; on serverless runtimes the Lambda could freeze after the response was sent and drop the in-flight Supabase insert, leaving the photo at `user_photos.status='pending'` with no `moderation_queue` row — invisible to moderators and stuck forever (repro: upload 3 photos, only the last reached the queue). The enqueue is now awaited and throws on failure (`src/lib/moderation/queue.ts`), the photo POST rolls back the `user_photos` row and the Supabase Storage object if queuing fails so the user can retry cleanly (`src/app/api/profile/photos/route.ts`), and the user-report caller wraps its now-throwing call in a try/catch to preserve its non-blocking behavior (`src/app/api/users/report/route.ts`). One-time reconciliation SQL documented in `SUPABASE_SETUP.md` re-queues any photos already orphaned by the bug.
- Dashboard and explore now rescale over the same candidate pool. `generateMatchesFromSupabase` was filtering out rows with `score < 0.10` before calling `rescaleCohortScores` (a pre-rescale floor meant to drop ~3% noise from raw MBA output), so the matches route rescaled a smaller cohort than `/api/attendees/search` — the same user pair could read 53% STRATEGIC on the dashboard and 83% STRATEGIC in explore with no other changes. The floor is now obsolete (rescaling always maps to `[0.10, 0.95]`, so 3% noise cannot surface) and has been deleted; both routes rescale over their full candidate set. Dashboard matches now match explore percentages for every shared candidate (`src/app/api/matches/route.ts`)
- Real matches no longer get buried on the dashboard by incomplete-profile placeholders. `generateMatchesFromSupabase` was handing candidates without usable questionnaires a hardcoded `0.50` fallback score (`0.65` when the viewer also lacked a questionnaire). With only a handful of testers, real MBA totals sit in the 0.20–0.35 band, so the synthetic 0.50s outranked every genuine match and `rescaleCohortScores` mapped them all to 74% STRATEGIC, pushing the viewer's actual best match down to 38% HIGH AFFINITY. Matched parity with `/api/attendees/search`: always score via `calculateMatchScore` so sparse or empty questionnaires produce naturally low totals (0 when fully empty) that rescale to the cohort floor — no hardcoded placeholder survives into the ranked list (`src/app/api/matches/route.ts`)
- Match cards on the dashboard no longer clip at the top when hovered. The `.hover-lift` utility translates each card up 2px, but the `DesktopMatchPageGrid` cell wrapper had `overflow-hidden` and the outer horizontal pager's `overflow-x-auto overflow-y-visible` was coerced to `overflow-y: auto` (CSS spec rule for mixed-axis overflow), so the lift was clipped by two stacked boundaries. Removed the inner wrapper's `overflow-hidden` (the `MatchCard` carousel variant already clips its own content via `h-full min-h-0 overflow-hidden`) and added `pt-2` to the pager to absorb the translate plus the hover shadow's upward blur (`src/components/dashboard/matches-grid.tsx`)
...
- Explore/Search cards now show the same AI-generated conversation starters as the Matches page. Previously, the dashboard fired a progressive enrichment loop against `POST /api/matches/[id]/starters` after the initial `/api/matches` load, but the Explore page only did the single `/api/attendees/search` round-trip and its `AttendeeSearchResult` type had no `conversationStarters` field, so the same attendee (e.g., Camila) rendered a Gemini-authored line on the dashboard and a template fallback (`"Hi Camila — Austin here. I'm in a similar lane with Builder…"`) on Explore. `AttendeeSearchResult` now carries an optional `conversationStarters` populated client-side; `ExploreContainer` runs an `enrichSearchStartersProgressively` loop (concurrency 2) after each search, reusing the existing starters endpoint (its path segment is ignored — caching keys off `viewerId + matchedUserId`), with a `searchTokenRef` cancellation token so stale AI results from a previous filter/keyword/page don't splat onto the current result set. `AttendeeCard.displayStarters` prefers `attendee.conversationStarters` when populated and falls back to `buildPersonalizedConversationStarters()` otherwise, so cards are never blank while AI is in flight or the provider is cooled down (`src/types/index.ts`, `src/components/explore/attendee-card.tsx`, `src/components/explore/explore-container.tsx`)
- Align the explore-enrichment payload with the matches route for attendees with zero MBA commonalities, so both pages hit the same starter-cache row. `/api/matches` injects a synthetic fallback commonality (`"${title} at ${company || 'their organization'}"`, or `"Fellow conference attendee"`) when the MBA produces no shared context, and the starters cache-version hash includes the commonalities list — so attendees like P.Seelam and ARIMBU whose only "Why connect" was the title-at-company fallback had a different cache key between the two paths. Dashboard generated + persisted the AI starters under the fallback key; Explore sent `commonalities: []`, missed both the in-memory and Supabase cache, tried to regenerate, and fell back to templates when the free-tier cooldown was already active from the dashboard load. `enrichSearchStartersProgressively` now mirrors the matches-route fallback via a local `fallbackCommonalityDescriptions()` helper, so Explore either hits the already-persisted row or generates with the same inputs the dashboard would (`src/components/explore/explore-container.tsx`)
